const questionImage =  (store, ques) => {
    console.log("----------");
     const images = fetch(`/api/user/dashboard/questions/my_questions?userId=${4}`)
      .then((res) => res.json())
     // .then((ques)=>console.log(ques));
      .then((ques)=> {for(let i in ques){
         const a=`<li class="list-group-item"><a href="./edit_question.html?${ques[i].id}">${ques[i].description}</a></li><br>`
